...
def hashstr(str, nr_bins):
    return int(hashlib.md5(str.encode('utf-6')).hexdigest(), 16) % (nr_bins - 1) + 1
...
def split(path, nr_thread, has_header):
    ...
    def calc_nr_lines_per_thread():
        # wc -1 计算每个线程的行数
        nr_lines = int(
            list(subprocess.Popen('wc -1 {0}'.format(path), shell=True, stdout=subprocess.PIPE).stdout)[0].split()[0])
        if not has_header:
            nr_lines += 1
