...
def logloss(p, y):
    p = max(min(p, 1. - 10e-12), 10e-12)  # 限制 0 < p < 1
    return -log(p) if y == 1. else -log(1. - p)


# B. Apply hash trick of the original csv now
...
def update_w(w, n, x, p, y):
    for i in x:
        # alpha / (sqrt(n) + 1) is the adaptive learning rate heuristic 启发式
...
for t, row in enumerate(DictReader(open(train))):
    ...
    # for progress validation, useliss for learning our model
    loss += logloss(p, y)
...
with open(result_path + 'submission.csv', 'w') as submission:
